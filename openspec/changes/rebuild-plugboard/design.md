...
**The harness is not covered.** Writing this decision surfaced 37 tracked paths under `.claude/` that this repository did not author: six skills declaring `license: MIT` and `author: openspec` in their own frontmatter, and twenty-five skill directories plus six slash commands declaring no licence, no copyright holder and no source URL. `ci/vault.json` already exempts `.claude` from the note gates on the ground that upstream owns its format, but that exemption is about classification and settles nothing about copyright, and no gate looks at this. Extending a blanket Apache-2.0 claim over them would reproduce the exact finding the supply-chain rule logs against the reference — "85 JPEGs of unestablished origin under a blanket MIT claim with no attribution" — inside the repository that logs it. So the grant is scoped and the gap is recorded, which is the refuse-never-degrade posture applied to a licence claim.
