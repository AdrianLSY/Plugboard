...
[the tunnel](the-tunnel.md). Reserving a frame type costs nothing; adding one later splits the
fleet.
...
**SSE** is [Primitive 1](three-primitives.md) where the response half never ends:
`Content-Type: text/event-stream`, no `Content-Length`, incremental flush per event, connection held
for hours, `retry` and `Last-Event-ID` reconnection semantics. Any intermediary buffering kills it.
...
**Needs zero new tunnel primitives.** It needs [Primitive 1](three-primitives.md) to stop being
method-allowlisted, JSON-string-bodied, whole-response-buffered, and path-normalised.
...
- **`draft-ietf-webtrans-http2` is a blueprint, not just an argument.** See
  [the tunnel](the-tunnel.md) — it is a ready-made capsule wire format for this exact tunnel
  shape.
...
combined. What the edge owns and never relays is [topology](topology.md)'s subject; this section is
what the protocols demand of it.
...
- **Rate limiting keyed on something authenticated.** The reference keys its API limiter on a
  client-supplied `x-api-key` header (`reference/Plugboard/lib/plugboard_web/plugs/rate_limiter.ex:86-91`) that no endpoint authenticates
...
## Where this behaviour is specified

This note is orientation. It argues *why* fidelity is the feature, and which properties are free
today and unfixable the moment a tenant pins a sidecar version; it states no requirement of its own.
Each property above is owned by a specification, and the specification wins wherever it and this note
disagree. Read [architecture](architecture.md) for how the pieces fit, and
[the register](../../openspec/changes/rebuild-plugboard/design.md) for the decisions behind them.

- **Opaque bodies, header fields as an ordered sequence of pairs, method tokens carried unmodified,
  raw request targets, the frame vocabulary, credit windows, the capability set and refusal rather
  than degradation** — [tunnel/wire-contract](../../openspec/changes/rebuild-plugboard/specs/tunnel/wire-contract/spec.md)
- **The separate timeout bounds, the per-route class including the unbounded total, time to first
  byte, idle between octets, a response body that never ends, admission against occupancy rather
  than arrival rate, and drain** — [proxy/http-fidelity](../../openspec/changes/rebuild-plugboard/specs/proxy/http-fidelity/spec.md)
- **`CONNECT` refused rather than forwarded and `TRACE` answered at the edge, per-hop framing, hop-by-hop
  removal including what `Connection` names, `Host` bound to SNI, client identity established by the
  proxy** — [proxy/edge-hygiene](../../openspec/changes/rebuild-plugboard/specs/proxy/edge-hygiene/spec.md)
- **Deferred 101, subprotocol negotiation by the backend, per-hop handshake fields and masking,
  per-hop compression, opaque frame payloads, fragmentation and control-frame interleaving, liveness
  per hop and end to end** — [proxy/websocket](../../openspec/changes/rebuild-plugboard/specs/proxy/websocket/spec.md)
- **The affinity key as bounded registry state, and eligibility filtered by capability before
  selection** — [tunnel/sidecar-registry](../../openspec/changes/rebuild-plugboard/specs/tunnel/sidecar-registry/spec.md)
- **Per-tenant certificates, ownership verification before issuance, renewal ahead of expiry, and
  bounded issuance failure** — [routing/custom-domains](../../openspec/changes/rebuild-plugboard/specs/routing/custom-domains/spec.md)
- **Matching on undecoded segment boundaries — which is why `%2F` inside a segment survives — and
  the forwarded remainder** — [routing/mount-points](../../openspec/changes/rebuild-plugboard/specs/routing/mount-points/spec.md)
- **Per-tenant bounds on every dimension, which is what a stream quota is, and traffic never
  crossing a tenant boundary** — [tenancy/isolation](../../openspec/changes/rebuild-plugboard/specs/tenancy/isolation/spec.md)
- **Octet fidelity; field, method and target fidelity; duration, exhaustion, interleaving and
  abandonment — as adversarial cases written before the code they gate** —
  [tunnel/conformance](../../openspec/changes/rebuild-plugboard/specs/tunnel/conformance/spec.md)

Two of the positions above are settled decisions rather than live arguments, and the register holds
both: the timeout taxonomy is [D7](../decisions/d07-timeout-taxonomy.md), and the WebTransport
verdict — reserved in the contract, shipped later as a separate terminator — is
