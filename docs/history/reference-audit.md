...
**Every citation in this note points into `reference/`, which is gitignored and not present in this
repository.** Nothing below is checkable until the prior art is on disk at the two revisions the
audit was taken against — `reference/Plugboard` at `6756a07` and `reference/Telephone` at `9ccba86`.
A line number resolved against any other revision is not the line this note cites. The note
recording how to obtain both at those revisions is written by task 4.2 of the
`restructure-docs-as-vault` change; it is deliberately not linked from here yet, because a link to a
file that does not exist fails the link gate.
...
- `reference/Plugboard/config/test.exs:4` — `config :argon2_elixir, t_cost: 1, m_cost: 8`. `m_cost` 8 means 2^8 KiB
  against a library default exponent of 16, removing exactly the cost that makes the linear token scan
  dangerous.
- `reference/Plugboard/config/test.exs:96` — `config :plugboard, :allow_localhost_hooks, true`, whose own comment says
