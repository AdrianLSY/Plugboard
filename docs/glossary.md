...
| **node** | An entry in the mount-point hierarchy, and never a running process — reserved that way for the whole set by [D20](decisions/d20-multi-instance.md). | A running proxy process, in the *title* of D20 itself: the register entry and its note are both headed "The proxy is a multi-node installation in v1", which is the one use of the word the decision's own body forbids. The body is right and the title is the defect; correcting it belongs to the register, not here. |
