...
class CreateTests(unittest.TestCase):
    def run_with_body(self, body: str, *extra: str) -> int:
        with tempfile.TemporaryDirectory() as temp:
            ...
            return create.main(
                ["--title", "Guard PRs", "--body-file", str(path), *extra]
            )
    ...
    def test_docs_check_stops_before_push_or_pr_create(self):
        ...
        with (
            patch.object(create, "command", side_effect=outputs) as command,
            patch.object(
                create.checks, "changed_paths", return_value=["contract/wire.proto"]
            ),
        ):
            result = self.run_with_body(
                VALID_BODY.replace(
        ...
        self.assertEqual(result, 1)
        self.assertEqual(
            command.call_count, 3
        )  # branch, repository, fetch; never push/create
    ...
    def test_valid_description_pushes_then_creates_pr(self):
        ...
        with (
            patch.object(create, "command", side_effect=outputs) as command,
            patch.object(
                create.checks, "changed_paths", return_value=["ci/pr/create.py"]
            ),
        ):
