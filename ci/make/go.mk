...
.PHONY: fmt stamp warm lint test test-fast test-integration test-conformance gen dev
...
# Build provenance, written from the ONE place that computes it (ci/stamp.py).
# The generated file is a build artifact: gitignored, never committed, and
# regenerated by every target that compiles. A component that computed its own
# would drift from the others and each would look self-consistent while doing it.
STAMP_DIR := internal/buildstamp
STAMP     := $(STAMP_DIR)/stamp.go

stamp:
	@mkdir -p $(STAMP_DIR)
	@python3 $(REPO_ROOT)/ci/stamp.py --component $(notdir $(CURDIR)) --language go > $(STAMP)
